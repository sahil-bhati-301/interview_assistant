...
const Analytics: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Full Width Performance Growth Trend */}
            ...
            <div className="space-y-4">
              ...
              <div className="relative h-80">
                ...
                <div className="absolute inset-0 ml-8">
                  <svg className="w-full h-full" viewBox={`0 0 ${Math.max(growthData.length * 80, 800)} 280`}>
                    {/* Line path */}
                    <path
                      d={growthData.map((point, index) =>
                        `${index === 0 ? 'M' : 'L'} ${index * 80 + 40} ${280 - (point.score / 100) * 280}`
                      ).join(' ')}
                      fill="none"
                      stroke="#3b82f6"
                      strokeWidth="4"
              ...
              <div className="text-center text-sm text-gray-600 dark:text-gray-400">
                Interview Number → Score Percentage (Hover over points for details)
              </div>
            </div>
          ) : (
      ...
      {/* Full Width Domain Performance Comparison */}
        ...
        <div className="h-96">
          {domainData.length > 0 ? (
            <div className="space-y-4">
              <div className="relative h-80">
                <div className="absolute inset-0 flex items-end justify-around px-8">
                  {domainData.map((domain, index) => {
                    const height = (domain.averageScore / 100) * 280;
                    // Use same colors as pie chart for consistency
                    const colors = [
                      '#3b82f6', // Blue
                      '#ef4444', // Red
                      '#10b981', // Green
                    ...
                    return (
                      <div key={index} className="flex flex-col items-center min-w-0 flex-1 max-w-32">
                        <div
                          className="w-full max-w-16 rounded-t mx-auto"
                          style={{
                            height: `${height}px`,
                        ...
                        <div className="text-xs text-gray-400 mt-1">
                          {domain.interviewCount} int{domain.interviewCount !== 1 ? 's' : ''}
                        </div>
                      </div>
                    );
                  })}
              ...
              <div className="text-center text-sm text-gray-600 dark:text-gray-400">
                Domain Performance Comparison → Average Score & Interview Count
              </div>
            </div>
          ) : (
            <div className="flex items-center justify-center h-full text-gray-500">
